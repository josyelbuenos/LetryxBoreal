      ...
      <div className="container relative z-10 mx-auto px-4">
        <div className="flex flex-col items-center text-center lg:flex-row lg:text-left gap-16">
          <div className={`flex-1 transition-all duration-1000 ${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-2 text-xs font-bold text-primary mb-8 ring-1 ring-primary/20 backdrop-blur-sm animate-reveal-right">
          ...
          <div className="flex-1 w-full max-w-2xl animate-fade-in-up animate-delay-500 opacity-0">
            <div className="relative aspect-video lg:aspect-square overflow-hidden rounded-[2rem] border-[12px] border-white/50 bg-card shadow-3xl animate-float group">
              {heroImage && (
                <Image
                  src={heroImage.imageUrl}
                  alt="Interface do Letryx Boreal"
                  fill
              ...
              <div className="absolute bottom-6 left-6 right-6 p-6 glass-effect rounded-2xl border border-white/30 backdrop-blur-xl translate-y-4 opacity-0 transition-all duration-700 group-hover:translate-y-0 group-hover:opacity-100">
                <p className="text-sm font-semibold text-primary">IA Contextual</p>
                <p className="text-xs text-muted-foreground">Analisa sentimentos e conexões literárias em tempo real.</p>
              </div>
            </div>
          </div>
        </div>
